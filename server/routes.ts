...
const clientAuthSession = new Map<string, string>();
...
const userJournal = new Map<string, Trade[]>();
const userAlerts = new Map<string, AlertRule[]>();
...
export async function registerRoutes(
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    clientAuthSession.set(clientId, user.username);
  ...
  app.get("/api/auth/me", async (req, res) => {
    const clientId = getClientId(req);
    const username = clientAuthSession.get(clientId);
  ...
  app.post("/api/auth/logout", (req, res) => {
    const clientId = getClientId(req);
    clientAuthSession.delete(clientId);
    return res.json({ ok: true });
  });
  app.get("/api/settings", (req, res) => {
    const clientId = getClientId(req);
    res.json(userSettings.get(clientId) ?? defaultSettings);
  });
  ...
  app.put("/api/settings", (req, res) => {
    ...
    const clientId = getClientId(req);
    userSettings.set(clientId, parsed.data);
    return res.json(parsed.data);
  });

  app.get("/api/watchlist", (req, res) => {
    const clientId = getClientId(req);
    return res.json({ watchlist: userWatchlists.get(clientId) ?? [] });
  });
  ...
  app.put("/api/watchlist", (req, res) => {
    ...
    const unique = Array.from(new Set(parsed.data.watchlist));
    const clientId = getClientId(req);
    userWatchlists.set(clientId, unique);
    return res.json({ watchlist: unique });
  });

  app.get("/api/journal", (req, res) => {
    const clientId = getClientId(req);
    return res.json({ trades: userJournal.get(clientId) ?? [] });
  });
  ...
  app.post("/api/journal", (req, res) => {
    ...
    const clientId = getClientId(req);
    const current = userJournal.get(clientId) ?? [];
    const next = [trade, ...current];
    userJournal.set(clientId, next);
  ...
  app.delete("/api/journal/:id", (req, res) => {
    const clientId = getClientId(req);
    const current = userJournal.get(clientId) ?? [];
    const next = current.filter((trade) => trade.id !== req.params.id);
    userJournal.set(clientId, next);
  ...
  app.get("/api/alerts", (req, res) => {
    const clientId = getClientId(req);
    return res.json({ alerts: userAlerts.get(clientId) ?? [] });
  });
  ...
  app.post("/api/alerts", (req, res) => {
    ...
    const clientId = getClientId(req);
    const current = userAlerts.get(clientId) ?? [];
    ...
    userAlerts.set(clientId, [rule, ...current]);
    return res.status(201).json(rule);
  });

  app.patch("/api/alerts/:id/trigger", (req, res) => {
    const clientId = getClientId(req);
    const current = userAlerts.get(clientId) ?? [];
  ...
  app.delete("/api/alerts/:id", (req, res) => {
    const clientId = getClientId(req);
    const current = userAlerts.get(clientId) ?? [];
    userAlerts.set(clientId, current.filter((rule) => rule.id !== req.params.id));
    return res.json({ deleted: req.params.id });
  });
